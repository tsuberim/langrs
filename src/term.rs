...
impl Term {
    ...
    pub fn free_vars(&self) -> HashSet<Id> {
        match self {
            ...
            Term::Record(_, items) => items
                .into_iter()
                .map(|(_k, v)| v.free_vars())
            ...
            Term::List(_, items) => items
                .into_iter()
                .map(|x| x.free_vars())
                .fold(hashset![], HashSet::union),
            Term::Access(_, x, _prop) => x.free_vars(),
            Term::App(_, f, args) => f.free_vars().union(
                args.into_iter()
                    .map(|x| x.free_vars())
                    .fold(hashset![], HashSet::union),
            ),
            Term::Block(_, _, defs, term) => {
                defs.iter().fold(term.free_vars(), |acc, (id, term)| {
                    acc.without(id).union(term.free_vars())
                })
            }
        ...
        match self {
            ...
            Term::Match(_, term, cases, default) => {
                let cases: Vec<String> = cases
                    .iter()
                    .map(|(tag, (id, result))| format!("{} {} -> {}", tag, id, result))
                    .collect();
                let default = if let Some(term) = default {
                    format!(" else {}", term)
                } else {
                    "".to_string()
                };
...
pub fn to_ast(
    node: Node,
    src: &Rope,
    cache: &mut HashMap<usize, Arc<Term>>,
    ...
    let term = match kind {
        ...
        "str" => {
            ...
            let terms = node
                ...
                .collect::<Result<Vec<Arc<Term>>, ParseError>>()?;

            let t = terms.into_iter().fold(
        ...
        "tag" => {
            ...
            let payload = if let Some(node) = payload {
                to_ast(node, src, cache)?
            } else {
                Arc::new(Term::Record(id, HashMap::new()))
            };
        ...
        "match" => {
            ...
            for (tag, (id, term)) in tags
                .into_iter()
                .zip(ids.into_iter().zip(results.into_iter()))
            {
        ...
        "curied_access" => {
            ...
            let term = Term::Lam(
                ...
                Arc::new(Term::Access(
                    id,
                    Arc::new(Term::Var(id, "x".to_string())),
                    property,
                )),
            );
            Ok(Arc::new(term))
                ...
                match stmt.kind() {
                    ...
                    "bind" => {
                        if defs.len() > 0 || type_defs.len() > 0 {
                            term = Arc::new(Term::Block(
                                id,
                                type_defs,
                                defs.iter().cloned().rev().collect(),
                                term,
                            ));
                            defs = vec![];
                            type_defs = hashmap! {};
                        ...
                        term = Arc::new(Term::App(
                            id,
                            Arc::new(Term::Var(id, "$".to_string())),
                            vec![rhs, Arc::new(Term::Lam(id, vec![lhs], term))],
                        ));
                    }
        ...
        _ => Err(ParseError { node_id: node.id(), message: "Invalid AST node".to_string() })
    }?;
